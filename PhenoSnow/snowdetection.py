...
class SnowDetection():
    ...
    def __init__(self,site,file_list,img_path,out_path,mask_inf,training_data,nu1,nu2,veg_type='SH',snow_baseline=0,random_state=11,write=True):
    ...
    def classify(self):
        ...
        sgdocsvm_1 = linear_model.SGDOneClassSVM(nu=self.nu1,random_state=self.random_state)
        sgdocsvm_1.fit(self.training_data[self.vec1])
        joblib.dump(sgdocsvm_1, self.out_path+self.site+'_sgdvm1.pkl')
        sgdocsvm_2 = linear_model.SGDOneClassSVM(nu=self.nu2,random_state=self.random_state)
        sgdocsvm_2.fit(self.training_data[self.vec2])
        joblib.dump(sgdocsvm_2, self.out_path+self.site+'_sgdvm2.pkl')
        ...
        for i in track(range(len(self.file_list)),description='Calculating Snow Ratio, Site:'+self.site):
            ...
            res1 = sgdocsvm_1.predict(data_frame[self.vec1])
            res1[res1 < 1] = 0
            res2 = sgdocsvm_2.predict(data_frame[self.vec2])
            res2[res2 < 1] = 0
            res3 = res1+res2
            res3[res3 > 1] = 1
            if self.write==True:
            ...
            recheck_data = new_data[(new_data['snow_ratio1'] > 0.01) & (new_data['snow_ratio2'] > 0.01)].copy()
            GBCC_snow90 = max(recheck_data['GBCC'].quantile(0.90),0)
            ...
            for i in range(len(new_data)):
                ...
                if self.veg_type == 'SH' or self.veg_type == 'DB':
                    ...
                    if new_data.loc[i, 'GBCC'] < GBCC_snow90:
        ...
        new_data.to_csv(self.out_path+self.site+'_Snow_Ratio.csv',index=False)
        
        return new_data[['site','year','month','day','time','snow_ratio']],GBCC_snow90
